var Display = {
  ...
  /**
   * threshold height value; anything smaller than this and it's considered that
   * the widget is in home screen mode
   */
  ...
  onResize : function() {
    ...
    if (Display.isTouch()) {
      // TODO maknuti na bolje mjesto, ovdje je da ne prijavljuje krivu velicinu
    ...
    if (newDims.height == Display.width && newDims.width == Display.height) {
      ...
      if (newWidgetSize.height < Display.MINI_VIEW_THRESHOLD) {
        Ext.getBody().addClass('home_screen')
        Ext.getBody().removeClass('normal_screen')
        Display.onEnterHomeView()
      } else {
        Ext.getBody().addClass('normal_screen')
        Ext.getBody().removeClass('home_screen')
