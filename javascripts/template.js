var T = {
  ...
  renderTemplate : function(name, data, callback) {
    if (T.cachedTemplates[name] == null) {
      T.loadTemplate(name, function() {
        callback(T.renderCachedTemplate(name, data))
      })
    } else {
      callback(T.renderCachedTemplate(name, data))
    }
  ...
  renderCachedTemplate : function(name, data) {
    return T.renderString(T.cachedTemplates[name], data)
  },
  ...
  renderCollection : function(name, collection, callback) {
    if (T.cachedTemplates[name] == null) {
      T.loadTemplate(name, function() {
        callback(T.renderCachedCollection(name, collection))
      })
    } else {
      callback(T.renderCachedCollection(name, collection))
    }
  ...
  renderCachedCollection : function(name, collection) {
    ...
    for ( var i in collection) {
      result += T.renderString(T.cachedTemplates[name], collection[i])
    }
    return result;
  },
  ...
  renderIn : function(holder, name, data, callback) {
    T.renderTemplate(name, data, function(rendered) {
      holder.html(T.renderCachedTemplate(name, data))
      if (callback) {
        callback()
      }
    })
  ...
  renderCollectionIn : function(holder, name, collection, callback) {
    T.renderCollection(name, collection, function(data) {
      holder.html(data)
      if (callback) {
        callback()
      }
  ...
  renderCachedCollectionIn : function(holder, name, collection) {
    holder.html(T.renderCachedCollection(name, collection))
  }
}
